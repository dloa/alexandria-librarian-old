...
let resourceDir = path.normalize(path.join(__dirname, '../../', 'bin', util.getOS(), 'scripts'));
module.exports = {
    enableStartOnBoot: function() {
        switch (util.getOS()) {
            case 'osx':
                return util.exec([
                    'osascript',
                    path.join(resourceDir, 'LoginItemAdd.scpt'),
                    'AlexandriaLibrarian'
                ]);
                break;
            case 'win':
    ...
    disableStartOnBoot: function() {
        switch (util.getOS()) {
            case 'osx':
                return util.exec([
                    'osascript',
                    path.join(resourceDir, 'LoginItemRemove.scpt'),
                    'AlexandriaLibrarian'
                ]);
                break;
            case 'win':
    ...
    statusStartOnBoot: function() {
        switch (util.getOS()) {
            case 'osx':
                return util.exec([
                    'osascript',
                    path.join(resourceDir, 'LoginItemCheck.scpt')
